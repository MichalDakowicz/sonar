...
export default function AlbumCard({ album, onClick, isHighlighted, innerRef, readOnly = false }) {
  ...
  const artistName = Array.isArray(album.artist) ? album.artist.join(", ") : album.artist;
  const formatName = Array.isArray(album.format) ? album.format.join(" • ") : (album.format || 'Digital');
  ...
  return (
    ...
    >
      ...
      {/* Gradient Overlay */}
      <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-transparent to-black/60 opacity-80 transition-opacity duration-300" />
              ...
              <div className="flex flex-row gap-1 items-end sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
                {!readOnly && (
                    <button
                        ...
                        className="flex items-center justify-center w-[22px] h-[22px] bg-white text-black rounded-full shadow-lg hover:bg-neutral-200 transition-colors"
                        title="Log Listen"
                    >
                ...
                {album.url && (
                    <a 
                        ...
                        className="flex items-center justify-center w-[22px] h-[22px] bg-[#1DB954] text-black rounded-full shadow-lg hover:bg-[#1ed760] transition-colors"
      ...
      <div className="absolute bottom-0 left-0 w-full p-3 z-20 flex flex-col justify-end">
        <h3 className="truncate font-bold text-white text-base drop-shadow-md leading-tight mb-0.5" title={album.title}>{album.title}</h3>
